...
names(fileNames) <- sapply(str_split(fileNames, "\\."), function(x){return(c(x[1]))})
...
roc <- ggplot(performance.frame, aes(x = FPR, y = TPR, color = group))+
    ...
    geom_vline(xintercept = 0.1, colour= "darkgrey") + guides(color = guide_legend(key_width = unit(1, "cm"), 
