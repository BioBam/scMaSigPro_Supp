...
inPath <- "supp/07_Different_Length_of_Pseudotime/data/input/sceObjects/"
outPath <- "supp/07_Different_Length_of_Pseudotime/data/input/inputFiles/"
...
for (i in names(fileNames)) {
  ...
  # Name of the List
  cat(paste("\nPreparing for:", str_list[[1]][2], "and", str_list[[1]][4]))
  ...
  raw_counts_file_name <- paste0(
    outPath, "rawCounts/", str_list[[1]][1], "_", str_list[[1]][2], "_",
    str_list[[1]][3], "_", str_list[[1]][4], "_rawCounts.tsv"
  )
  ...
  # Save data
  cell_meta_file_name <- paste0(
    outPath, "rawMeta/", str_list[[1]][1], "_", str_list[[1]][2], "_",
    str_list[[1]][3], "_", str_list[[1]][4], "_rawMeta.tsv"
  )
  ...
  gene.meta <- gene.meta[, !(colnames(gene.meta) %in% c(
  ...
  # Create Files
  gene_meta_file_name <- paste0(
    outPath, "rawGT/", str_list[[1]][1], "_", str_list[[1]][2], "_",
    str_list[[1]][3], "_", str_list[[1]][4], "_rawGT.tsv"
  )
