...
# Split String
fileNames <- sapply(str_split(fileNames, "_"), function(x) {
  return(paste(c(x[1], x[2], x[3], x[4]), collapse = "_"))
})
...
for (i in names(fileNames)) {
  ...
  rawCounts <- as.matrix(read.table(
    paste0(
      inPath, "rawCounts/",
  ...
  bulk_counts_list <- make_bulk_counts(
    counts = rawCounts, cluster_member_col = "cluster.members",
  ...
  # Create an Object
  pseudobulk.data.input <- list(
    pbCounts = as.matrix(bulk_counts),
  ...
  saveRDS(
    pseudobulk.data.input,
    paste0(
      outPath, str_list[[1]][1], "_", str_list[[1]][2],
      "_", str_list[[1]][3],
      "_", str_list[[1]][4], ".RDS"
    )
  )
