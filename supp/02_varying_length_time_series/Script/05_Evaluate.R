...
# File-Location
gtPath <- "supp/02_varying_length_time_series/data/input/PseudobulkInput/"
inPath <- "supp/02_varying_length_time_series/data/output/MaSigPro/tstep/"
outPath <- "supp/02_varying_length_time_series/data/output/MaSigPro/ComparisonEvaluate/"
...
for (i in names(fileNames)) {
    ...
    # Add Column of Test
    df.performance$length = str_remove(str_split(i, "_")[[1]][2], ".RDS")
    
    # Add to list
    performance.list[[str_remove(str_split(i, "_")[[1]][2], ".RDS")]] <- df.performance
}
...
# ROC
roc <- ggplot(performance.frame, aes(x = FPR, y = TPR, color = length))+
    geom_point()+
    geom_path(linewidth = 1.5, alpha = 0.6)+
