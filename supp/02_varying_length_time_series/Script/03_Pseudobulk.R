...
inPath <- "supp/02_varying_length_time_series/data/input/inputFiles/"
outPath <- "supp/02_varying_length_time_series/data/input/PseudobulkInput/"

# Load filenames
fileNames <- list.files(paste0(inPath,"rawMeta"))

# Split String
fileNames <- sapply(str_split(fileNames, "_"), function(x){return(paste(c(x[1], x[2]), collapse = "_"))})
...
for (i in names(fileNames)) {
    ...
    # Name of the List
    cat(paste("\nPreparing for Length:", str_list[[1]][2]))
    
    # Add Compression Columns for Pseudobulking
    pbMeta <- entropy_discretize(rawMeta, time_col = "Pseudotime", drop.fac = 0.6)
    ...
    saveRDS(pseudobulk.data.input,
            paste0(outPath, str_list[[1]][1], "_", str_list[[1]][2], ".RDS"))
    
    
    # cell_metadata_bulk_name_image <- paste0("inputBulkCompressionInfo/", 
    #                                         test_name, "_",
    #                                         test_value, "_",
    #                                         "CompressInfo", 
    #                                         "_df_", bulk_discretize_list[["drop.fac"]],
    #                                         "_Sturges","_sum", ".png")
    # png(filename = cell_metadata_bulk_name_image, width = 500, height = 500)
    # hist(pbMeta$bin_size)
    # dev.off()
    }
