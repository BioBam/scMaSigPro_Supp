...
plot_cells(cds, show_trajectory_graph = F, color_cells_by = "Batch",
...
path_lengths <- apply(cellWeights, 2, FUN = function(x){length(x[x==1])})
...
plot_cells(cds_lin, show_trajectory_graph = T, color_cells_by = "Lineage",
           cell_size = 2, label_roots = T, label_principal_points = T)
plot_cells(cds_lin, show_trajectory_graph = T, color_cells_by = "pseudotime",
           cell_size = 2, label_roots = T, label_principal_points = T)
...
bulk_cell_metadata <- make_pseudobulk_design(design.file = pbProfile,addBinSize = T,
                                             paths.vector = c("Lin1", "Lin2"),
                                             binnedCol = "binnedTime")

#Derive counts
bulk_counts_list <- make_bulk_counts(counts = as.matrix(cds_lin@assays@data@listData$counts),
...
library(MASS)
p.vec <- p.vector(sumCounts, design = design,
         counts = T)
t.fit <- T.fit(p.vec, design = design$dis,
               family = p.vec$family)

sigs <- get.siggenes(t.fit, vars = "groups", rsq = 0.6)
suma2Venn(sigs$summary[, c(1:2)])

PlotGroups(data =sumCounts[91,], edesign = design$edesign, show.fit = T,
