...
# File-Location
gtPath <- "supp/03_varying_no_cells/data/input/PseudobulkInput/"
inPath <- "supp/03_varying_no_cells/data/output/MaSigPro/tstep/"
outPath <- "supp/03_varying_no_cells/data/output/MaSigPro/ComparisonEvaluate/"
...
for (i in names(fileNames)) {
    ...
    # Add Column of Test
    df.performance$skew = str_remove(str_split(i, "_")[[1]][2], ".RDS")
...
# ROC
roc <- ggplot(performance.frame, aes(x = FPR, y = TPR, color = skew))+
    geom_point()+
    geom_path(linewidth = 1.5, alpha = 0.6)+
