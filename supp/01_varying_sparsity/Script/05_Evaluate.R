...
# File-Location
gtPath <- "supp/01_varying_sparsity/data/input/PseudobulkInput/"
inPath <- "supp/01_varying_sparsity/data/output/MaSigPro/tstep/"
outPath <- "supp/01_varying_sparsity/data/output/MaSigPro/ComparisonEvaluate/"
...
for (i in names(fileNames)) {
    ...
    # Load tstep
    tstepObj <- readRDS(paste0(inPath, i))
    
    # Load ground truth
    gt <- readRDS(paste0(gtPath, i))
    groundTruth <- gt$gt
    ...
    # Add Column of Test
    df.performance$zi = str_remove(str_split(i, "_")[[1]][2], ".RDS")
    
    # Add Type
    df.performance$Type <- ifelse(str_split(i, "_")[[1]][4] == 0, "Mid:0", "Shape:-1")
    
    # Add to list
    performance.list <- append(performance.list, list(df.performance))
}
...
# Extract the Frame
performance.frame <- performance.frame[performance.frame$Type == "Mid:0",]

# Comparitive ROC
acc<- ggplot(performance.frame, aes(x = VARIABLE, y = ACCURACY,
                        color = zi))+
    geom_point()+
...
# ROC
roc <- ggplot(performance.frame, aes(x = FPR, y = TPR, color = zi))+
    geom_point()+
    geom_path(linewidth = 1.5, alpha = 0.6)+
...
ggsave(acc, filename = paste0(outPath, "acc.png"),
       dpi = 600, height = 8, width = 8)
ggsave(roc, filename = paste0(outPath, "roc.png"),
       dpi = 600, height = 8, width = 8)
