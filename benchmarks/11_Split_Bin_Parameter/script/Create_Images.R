...
evaluation.frame$SplitBinParam <- str_split_i(evaluation.frame$SplitBinbSkew, "_", i = 2)
evaluation.frame$skew <- str_split_i(evaluation.frame$SplitBinbSkew, "_", i = 1)
...
roc <- ggplot(evaluation.frame, aes(x = FPR, y = TPR, color = as.factor(skew))) +
    ...
    geom_path(linewidth = 1, alpha = 0.7, aes(linetype = SplitBinParam)) +
...
acc <- ggplot(evaluation.frame, aes(
    ...
    geom_path(linewidth = 1, alpha = 0.6, aes(linetype = SplitBinParam)) +
    scale_color_manual(values = colorConesa(10))+
    labs(
...
long_data <- melt(evaluation.frame, id.vars = c("VARIABLE", "skew", "SplitBinParam"), measure.vars = c("ACCURACY", "PRECISION", "FPR", "TPR", "FNR"))

# Plot performance
performance <- ggplot(long_data, aes(x = VARIABLE, y = value, group = interaction(skew, variable, SplitBinParam), color = variable)) +
    geom_line(linewidth = 0.6) + 
    geom_point(size = 0.8) +
    scale_color_manual(values = colorConesa(5)) +
    facet_wrap(SplitBinParam~skew, scales = "free_y", nrow = 2, ncol = 4, 
