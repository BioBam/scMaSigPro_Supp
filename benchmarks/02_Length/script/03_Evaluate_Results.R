# Title: Evaluate the results of ScMaSigPro on simulated datasets with different levels of lenEq
...
dirPath <- "benchmarks/02_Length/data/output/"
...
for (i in names(dataSets)) {
  ...
  # Get Performance
  performance.measure <- get_performance(scmp_obj = scmp.obj,
...
write.table(evaluation.frame, paste0(dirPath, "Performance.Table.tsv"), sep = "\t",
            row.names = F, quote = F)

evaluation.frame$lenEq <- as.factor(as.numeric(evaluation.frame$lenEq))

# ROC
roc <- ggplot(evaluation.frame, aes(x = FPR, y = TPR, color = lenEq))+
    geom_point()+
    ...
    theme(panel.grid.major = element_line(linewidth = 0.7, color = "lightgrey", linetype = "dotted"),
...
# Accuracy
acc<- ggplot(evaluation.frame, aes(x = VARIABLE, y = ACCURACY,
                                    color = lenEq))+
    geom_point()+
...
ggsave(acc, filename = paste0(dirPath, "Accuracy.png"),
       dpi = 600, height = 8, width = 10)
ggsave(roc, filename = paste0(dirPath, "ROC.png"),
