...
inPath <- "/supp_data/benchmarks/07_BinningMethod/output/"
outPath <- "Tables/"
...
names(dataSets) <- str_remove(
  str_remove(dataSets, pattern = "scmp.obj.method."),
  ".RData"
)
...
for (i in names(dataSets)) {
  ...
  # Add to list
  performance.measure[["parameter"]] <- "Method"
...
# Write
write.table(evaluation.frame, paste0(outPath, "07_Method_Performance.Table.tsv"),
  sep = "\t",
  row.names = F, quote = F
)
