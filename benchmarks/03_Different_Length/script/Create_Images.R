...
# ROC Curve
roc <- ggplot(evaluation.frame, aes(x = FPR, y = TPR, color = as.factor(Arm))) +
    geom_point() +
    ...
    labs(
        title = "ROC-curve for Relative Path Length",
        subtitle = "Varying R2",
...
long_data <- melt(evaluation.frame, id.vars = c("VARIABLE", "Arm"), measure.vars = c("ACCURACY", "PRECISION", "FPR", "TPR", "FNR"))

# Plot performance
performance <- ggplot(long_data, aes(x = VARIABLE, y = value, group = interaction(Arm, variable), color = variable)) +
    geom_line(linewidth = 0.6) + 
    geom_point(size = 0.8) +
    scale_color_manual(values = colorConesa(12)) +
    facet_wrap(~Arm, scales = "free_y", nrow = 3, ncol = 4, 
               labeller = labeller(Zi = function(x) paste("Path Length", x))) +
