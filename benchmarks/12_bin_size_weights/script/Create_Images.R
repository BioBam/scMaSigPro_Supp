...
dirPath <- "benchmarks/12_bin_size_weights/data/output/"
...
acc <- ggplot(evaluation.frame, aes(
    ...
    color = as.factor(UseBinSizeWeights)
)) +
    geom_point() +
...
long_data <- melt(evaluation.frame, id.vars = c("VARIABLE", "UseBinSizeWeights"), measure.vars = c("ACCURACY", "PRECISION", "FPR", "TPR", "FNR"))

# Plot performance
performance <- ggplot(long_data, aes(x = VARIABLE, y = value, group = interaction(UseBinSizeWeights, variable), color = variable)) +
    geom_line(linewidth = 0.6) + 
    geom_point(size = 0.8) +
    scale_color_manual(values = colorConesa(5)) +
    facet_wrap(~UseBinSizeWeights, scales = "free_y", nrow = 2, ncol = 4, 
