...
sim.sce <- readRDS("../supp/01_varying_sparsity/data/input/sceObjects/zi_mid_10_mid_0_shape_-1.25.RDS")
```
...
raw.meta <- cell.meta.df %>% mutate(Replicate = data.table::rleid(
                                           Step, Path1, Path2)) %>% as.data.frame()
...
pbMeta <- entropy_discretize(raw.meta, time_col = "Pseudotime", drop.fac = 0.6,
                             method = "Sturges")
...
design.profile <- make_pseudobulk_design(design.file = pbMeta,
...
bulk_counts_list <- make_bulk_counts(counts = raw_counts, cluster_member_col = "cluster.members", 
...
#install.packages("../../scMaSigPro_0.1.6.tar.gz", repos = NULL, type = "source")
library(scMaSigPro)
```
...
# Entropy
pb.meta.scmp <- scMaSigPro::entropy_discretize(design_table = raw.meta.scmp,
...
design.profile.scmp <- scMaSigPro::make.pseudobulk.design(pb.meta.scmp,
                                         "Path")
```
...
## Make Bulk-Counts
```{r}
bulk_counts_sum.scmp <- scMaSigPro::make.pseudobulk.counts(counts = raw_counts,
                                   cluster_member_col = "cluster.members",
                                   bin_col = "bin", pseudo_bulk_profile = design.profile.scmp,
...
```{r}
scmp.obj <- sc.p.vector(scmpObj = scmp.obj, verbose = F, min.obs = 6,
                        counts = T, theta = 10)
```


```{r}
scmp.obj <- sc.T.fit(data = scmp.obj, verbose = F,
                     step.method = "backward", family = scmp.obj@scPVector@family)
```
...
testthat::expect_equal(as.data.frame(sig.profiles.tstep.scmp),
...
testthat::expect_equal(as.data.frame(group.coeffs.tstep.scmp),
...
testthat::expect_equal(as.data.frame(edesign.tstep.scmp),
