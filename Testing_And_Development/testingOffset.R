...
groundTruth <- read.table(
  file = "benchmarking_Offset/data/input/GroundTruth/zi_shape_1_GroundTruth.tsv",
...
write.table(df.performance, measure_name,
  row.names = F, sep = "\t",
  quote = F
)
...
# ROC
a <- ggplot(df.performance, aes(
  x = FPR, y = TPR, linetype = Offset,
  color = ZI
)) +
  geom_point() +
