get.siggenes_test<-function (tstep, rsq = 0.7, add.IDs = FALSE, IDs = NULL, matchID.col = 1, 
        ...
        (matchID.col + 1)], match = IDs[, matchID.col], r = 0.7) 
{
    ...
    # If the Any R-square is greater 
    if (any(tstep$sol[, 2] > rsq)) {
        ...
        if (vars == "all") {
            ...
            # View(group.sig)
            # stop()
            #return(list(sigs= sigs, coeffs = coeffs, gc = gc, sig.pvalues= sig.pvalues))
