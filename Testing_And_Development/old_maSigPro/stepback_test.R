stepback_test <- function (y = y, d = d, alfa = 0.05, family = gaussian() , epsilon=0.00001,
                      offsetData = offsetData) 
{
    #lm1 <- glm(y ~ ., data = d, family=family, epsilon=epsilon)
    lm1 <- glm(y ~ ., data = d, family=family, epsilon=epsilon, offset = offsetData)
    result <- summary(lm1)
    max <- max(result$coefficients[, 4][-1], na.rm = TRUE)
    if (length(result$coefficients[, 4][-1]) == 1) {
      if (max > alfa) {
        max = 0 
        lm1 <- glm(y ~ 1,  family=family, epsilon=epsilon, offset = offsetData)
