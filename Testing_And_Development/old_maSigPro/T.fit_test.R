T.fit_test <- function (data, design = data$dis, step.method = "backward", 
    min.obs = data$min.obs, alfa = data$Q, nvar.correction = FALSE, family=gaussian(), epsilon=0.00001, item="gene" ) 
{
    ...
    offsetData <- log(DESeq2::estimateSizeFactorsForMatrix(dat +1 ))
    ...
    for (i in 2:(g + 1)) {
        ...
        if (step.method == "backward") {
            reg <- stepback_test(y = y, d = dis, alfa = alfa, family=family, epsilon=epsilon,
