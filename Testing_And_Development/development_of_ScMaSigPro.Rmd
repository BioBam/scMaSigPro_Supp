...
sim.sce <- readRDS("../supp/01_varying_sparsity/data/input/sceObjects/zi_mid_60_mid_0_shape_0.25.RDS")
```
...
scmp.obj <- sc.p.vector(scmpObj = scmp.obj, verbose = F, min.obs = 6,
                        counts = T, theta = 1, epsilon = 0.00001)
```
...
# Running T-fit
```{r,  eval=FALSE}
scmp.obj <- sc.T.fit(scmp.obj, verbose = F, step.method = "backward", family = 
                         scmp.obj@scPVector@family)
```
...
# Load Data of skew_density_0.5.RDS
```{r}
tstep.long <- readRDS("../scMaSigPro_Supp/supp/01_varying_sparsity/data/output/MaSigPro/tstep/zi_60_mid_0_shape_0.25.RDS")
sol <- tstep.long$sol

# Equate Column names
new.sol <- scmp.obj@scTFit@sol
colnames(new.sol) <- colnames(sol)

testthat::expect_equal(new.sol, sol)
```
