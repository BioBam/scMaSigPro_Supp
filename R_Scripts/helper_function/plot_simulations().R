...
plot_simulations <- function(sim.sce, 
    ...
    sim.sce <- logNormCounts(sim.sce, transform ="log",
    ...
    # Require Plotly 
    if (plot3d == T){
    p1 <- plot_ly(threeD_PCA, x = ~PC1, y = ~PC2, z = ~PC3, color = ~.data[[colorGroupContinuous]],  alpha =0.7,mode="markers", 
                  size = 2) %>%
        layout(title = paste(title.3d, "color by", colorGroupContinuous), plot_bgcolor = "#e5ecf6")
    
    p2 <- plot_ly(threeD_PCA, x = ~PC1, y = ~PC2, z = ~PC3, color = ~.data[[colorGroupDiscrete]],  mode="markers", alpha =0.7,
                  size = 2)%>% layout(title = paste(title.3d, "color by" , colorGroupDiscrete), plot_bgcolor = "#e5ecf6",
...
plot_cell_time_distribution <- function(sce_object,
    ...
    plt.table <- plt.table %>% group_by(.dots = c(time_col,path_col)) %>% summarise(cluster.members=paste0(Cell, collapse='|'))
    ...
    p <- ggplot(plt.table.1, aes(x=Num)) + 
        ...
        theme_classic() + theme(legend.position = "none", strip.text = element_text(size=rel(2)),
        ...
        ylab("Number of cells") + xlab("Number of cells associations")
