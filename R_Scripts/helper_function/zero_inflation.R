...
plot_zero_density_per_cell <- function(sim_mat, precision = 2){
    plt.table <- data.frame(Num = per_col_zero(sim_mat),
               cell_id = names(per_col_zero(sim_mat)))
    # Plot Data
    p <- ggplot(plt.table, aes(x=Num)) + geom_density(
        color="#f68a53",
        ...
        ylab("Number of cells") + xlab("Percentage of Zero")
    print(p)
}
...
plot_zero_hist_per_cell <- function(sim_mat, precision = 2){
    plt.table <- data.frame(Num = per_col_zero(sim_mat),
                            cell_id = names(per_col_zero(sim_mat)))
    # Plot Data
    p <- ggplot(plt.table, aes(x=Num)) + geom_histogram(
