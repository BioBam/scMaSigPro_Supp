...
sob.tc <- NormalizeData(sob.tc, normalization.method = "LogNormalize", 
                     scale.factor = 10000, verbose = F)
sob.sim <- NormalizeData(sob.sim, normalization.method = "LogNormalize", 
                        scale.factor = 10000, verbose = F)
sob.tc <- FindVariableFeatures(sob.tc, selection.method = "vst", nfeatures = 2000,
                            verbose = F)
sob.sim <- FindVariableFeatures(sob.sim, selection.method = "vst", nfeatures = 2000,
                               verbose = F)
...
plt.sim <- ggplot(plt.data.sim) +
    ...
    ggtitle(
        ...
        subtitle = paste("True Sparsity:", trueSparsity,
