...
ggsave(plot =general.plot, filename = paste0(imgDir
...
similar.change.de <- plot_loess_fit(sce_obj = sim.sce, "Gene14", dfreedom = 1, log = T,
                                    plt_subtitle = "Differentially Expressed")
opposite.change.de <- plot_loess_fit(sce_obj = sim.sce, "Gene691", dfreedom = 1, log = T,
                                     plt_subtitle = "Differentially Expressed")
one.change.de <- plot_loess_fit(sce_obj = sim.sce, "Gene67", dfreedom = 1, log = T,
                                plt_subtitle = "Differentially Expressed")
no.change <- plot_loess_fit(sce_obj = sim.sce, "Gene1", dfreedom = 1, log = T,
                            plt_subtitle = "Not-Differentially Expressed")

gt.true <- ggarrange(similar.change.de, opposite.change.de, one.change.de, no.change,
          labels = c("A.", "B.", "C.","D."))

ggsave(plot = gt.true, filename = paste0(imgDir
